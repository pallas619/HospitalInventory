            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden">
                <form action="{{ route('medicines.store') }}" method="POST" class="p-6 space-y-6">
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        ...
                        <!-- Perlu Resep -->
                        <div class="space-y-2">
                            <label for="requires_prescription" class="text-sm font-medium text-gray-700">Perlu
                                Resep?</label>
                            <select id="requires_prescription" name="requires_prescription"
                                class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                <option value="0" {{ old('requires_prescription') == '0' ? 'selected' : '' }}>Tidak
                                </option>
                                <option value="1" {{ old('requires_prescription') == '1' ? 'selected' : '' }}>Ya
                                </option>
                            </select>
                        </div>
                    </div>
